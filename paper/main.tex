...
\begin{subfigure}[b]{\columnwidth}
  ...
  \caption{Standalone 1D-CNN magnetic sequence matcher (MAE 3.58~m).}
  \label{fig:cdf_mag}
\end{subfigure}\vspace{0.1cm}
\begin{subfigure}[b]{\columnwidth}
  \centering
  \includegraphics[width=0.85\columnwidth]{figures/kalman_full_wifi.png}
  \caption{Temporal fusion under full Wi-Fi conditions (1~Hz).}
  \label{fig:cdf_full}
\end{subfigure}\vspace{0.1cm}
\begin{subfigure}[b]{\columnwidth}
  \centering
  \includegraphics[width=0.85\columnwidth]{figures/kalman_degraded_wifi.png}
  \caption{Temporal fusion under degraded Wi-Fi (5~s gap, 40\% AP dropout).}
  \label{fig:cdf_degraded}
\end{subfigure}
...
Fig.~\ref{fig:trajectory} shows a representative test walk under the degraded Wi-Fi regime. The PDR-only trajectory (dashed red) diverges progressively from the ground truth (solid blue) as heading errors accumulate during the inter-scan gaps; the divergence is most visible in corridor turns where a small heading bias compounds over multiple steps. The Dual-Innovation KalmanNet (solid green), receiving scalar magnetic anomaly corrections at 16.7~Hz throughout each outage, suppresses this lateral drift and maintains close alignment with the true path. Upon each Wi-Fi fix arrival, the filter rapidly re-anchors, preventing further error accumulation.
